...
describe("Universal request logger", () => {
  ...
  function getLogs(): LogRow[] {
    return db.prepare("SELECT * FROM request_logs ORDER BY id ASC").all() as LogRow[];
  }
  ...
  it("disables Fastify console request logging while keeping SQL logging enabled", () => {
  ...
  it("logs the cache-busting image path exactly as requested", async () => {
    const apiKey = await registerDevice(app, DEVICE_MAC);
    db.prepare("DELETE FROM request_logs").run();
    ...
    const { image_url } = display.json<{ image_url: string }>();
    const imagePath = new URL(image_url).pathname;
    ...
    const logs = getLogs();
    const imageLog = logs.find((l) => l.path === imagePath);
    expect(imageLog).toBeDefined();
    expect(imageLog!.status_code).toBe(200);
  });
