...
export function createTestDB(): AppDB {
  return openDatabase(":memory:", TEST_CONFIG.refreshRateSeconds);
}
...
export function makeTestConfig(overrides: Partial<Config> = {}): Config {
  const refreshRateSeconds =
    overrides.refreshRateSeconds ?? TEST_CONFIG.refreshRateSeconds;

  return {
    ...TEST_CONFIG,
    ...overrides,
    refreshRateSeconds,
    refreshRateMs: overrides.refreshRateMs ?? refreshRateSeconds * 1000,
  };
}
...
export async function buildTestApp(
  ...
  const app = await buildApp(makeTestConfig(configOverrides), testDb);
  return app;
}

export async function buildTestDashboard(
  db?: AppDB,
  configOverrides: Partial<Config> = {}
): Promise<FastifyInstance> {
  const testDb = db ?? createTestDB();
  return buildDashboard(makeTestConfig(configOverrides), testDb);
}
